...
class LC_calibration_2(QWidget):
    ...
    def send_cal_thrust(self):
        try:
            cal_thrust_data = 'calSecondThrust'
    ...
    def update_data(self):
        thrust = float(app_globals.window.second_thrust_test_value)
        thrust_grams = format((float(app_globals.window.second_thrust_test_value) *1000.00) / g_const, '.2f')
        thr_weight = float(app_globals.window.second_thr_weight_test_value)
        torque = float(app_globals.window.second_torque_test_value)
        trq_weight = float(app_globals.window.second_trq_weight_test_value)
