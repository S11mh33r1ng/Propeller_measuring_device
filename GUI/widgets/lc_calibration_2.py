...
class LC_calibration_2(QWidget):
    ...
    def __init__(self, shared_data):
        ...
        self.setWindowTitle("2. posti koormusandurite kalibreerimine")
        ...
        self.torque_cal_val.setValue(self.shared_data.second_trq_cal_val)
        ...
        self.thrust_cal_val.setValue(self.shared_data.second_thr_cal_val)
        ...
        self.trq_arm_length.setValue(self.shared_data.second_trq_arm_length)
        ...
        self.thr_arm_length.setValue(self.shared_data.second_thr_arm_length)
    ...
    def send_cal_torque(self):
        try:
            cal_trq_data = 'calSecondTorque|%.2f' %(self.known_mass.value())
    ...
    def send_cal_thrust(self):
        try:
            cal_thrust_data = 'calSecondThrust|%.2f' %(self.known_mass.value())
    ...
    def update_data(self):
        try:
            ...
            thrust_mN  = float(getattr(w, "second_thrust_test_value", 0.0))
            torque_Nmm = float(getattr(w, "second_torque_test_value", 0.0))
    ...
    def send_thr_arm_length(self):
        thr_arm_length_data = 'setSecondThrArmLength|%.2f' %(self.thr_arm_length.value())
        self.sendData.emit(thr_arm_length_data)

    def closeEvent(self, event):
        """Ensure OFF command is sent when this calibration window closes."""
        try:
            app_globals.window.sendData('OFF')
        except Exception:
            pass
