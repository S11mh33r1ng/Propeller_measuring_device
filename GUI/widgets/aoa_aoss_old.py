...
#         layout3.addWidget(self.aoss_enabled_button)
    ...
    def send_aoa_trim(self):
        try:
            ...
            aoa_trim_data = 'trimAoA|%f' % (self.shared_data.aoa_trim)
    ...
    def send_read_aoa(self):
        try:
            # Command the MCU to reply with: readAoA|...|<value>|...
            self.sendData.emit("readAoA")
        except serial.SerialException as e:
            print(f"Error sending data: {e}")

    def set_read_aoa_value(self, val: str):
        """Update the widget label with the parsed AoA value."""
        try:
